...
export default function Modal() {
  ...
  const renderIngredients = () => {
    ...
    for(let i = 1; i <= 15; i++ ) { 
      //keyof can use any key from a type in this case we are using the keys from Recipe
      const ingredient = selectedRecipe[`strIngredient${i}` as keyof Recipe] 
      const measure = selectedRecipe[`strMeasure${i}` as keyof Recipe] 

      if( ingredient && measure ) {
        //Remember that in React is not recommendable using push, but that is only with states, we are working we an array
        ingredients.push(
                ...
                <DialogPanel className="relative transform overflow-hidden rounded-lg bg-white px-4 pt-5 pb-4 text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-2xl sm:p-6">
                  ...
                  >
                    ...
                    <div className="mt-5 flex justify-between gap-4">
                      ...
                      >{favoriteExist(selectedRecipe.idDrink) ? 'Remove from Favorites' : 'Add to Favorite'}</button>
                    </div>
                  </DialogTitle>
                </DialogPanel>
              </TransitionChild>
